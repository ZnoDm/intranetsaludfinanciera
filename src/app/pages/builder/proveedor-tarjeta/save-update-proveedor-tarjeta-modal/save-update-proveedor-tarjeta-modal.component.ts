...
@Component({
  selector: 'app-save-update-proveedor-tarjeta-modal',
  templateUrl: './save-update-proveedor-tarjeta-modal.component.html',
  styleUrls: ['./save-update-proveedor-tarjeta-modal.component.scss'],
  providers: [
    {provide: NgbDateAdapter, useClass: CustomAdapter},
    {provide: NgbDateParserFormatter, useClass: CustomDateParserFormatter}
  ]
})
export class SaveUpdateProveedorTarjetaModalComponent implements OnInit {
  ...
  save() {
    ...
    if(this.id == 0){
      this.proveedorTarjetaService.create(data).subscribe(
        (data:any) => {
          if (data.ok > 0) {
      ...
      this.proveedorTarjetaService.update(this.id,data).subscribe(
        (data:any) => {
          if (data.ok > 0) {
            this.toastr.successToastr(data.message, 'Correcto!', {
              toastTimeout: 2000,
